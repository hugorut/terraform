...
// Verify that plugin-dir doesn't prevent discovery of src providers
func TestInit_pluginDirWithBuiltIn(t *testing.T) {
	td := tempDir(t)
	testCopyDir(t, testFixturePath("init-src"), td)
	defer os.RemoveAll(td)
	defer testChdir(t, td)()
...
func TestInit_invalidBuiltInProviders(t *testing.T) {
	...
	//   not exist at all.
	td := tempDir(t)
	testCopyDir(t, testFixturePath("init-src-invalid"), td)
	defer os.RemoveAll(td)
	defer testChdir(t, td)()
